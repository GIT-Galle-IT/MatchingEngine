...
struct OrderMessage
{
    ...
    OrderMessage(std::string instrumentID_, int orderType_, int orderQuantity_, int price_) :
...
{
    os << message.instrumentID << " " << message.orderType << " " << 
        message.orderQuantity << " " << message.price;
    return os;
}